...
public class RemoveGroupCommand implements SubCommand {
    ...
    public RemoveGroupCommand(JavaPlugin plugin, WorldManager worldManager, GroupCommandHandler commandHandler) {
        this.plugin = plugin;
    ...
    public void execute(CommandSender sender, String[] args) {
        if (args.length < 2) {
            sender.sendMessage(ChatColor.RED + "Use '/group remove <group name> <world name>' to remove a world from a group.");
            return;
        }
        ...
        if (scopeMeta.getScopeId().equalsIgnoreCase(worldName)) {
            sender.sendMessage(ChatColor.RED + "You cannot remove a world from it's default group or it will be auto created again!");
            return;
        }

        // Determine if group exists
        if (!meta.getScopeID().equalsIgnoreCase(scopeMeta.getScopeId())) {
            sender.sendMessage(ChatColor.RED + "That group does not contain that world!");
            return;
        }
        ...
        sender.sendMessage(ChatColor.GOLD + "Removed world '" + worldName + "' from group: '" + scopeMeta.getName() + "'.");
